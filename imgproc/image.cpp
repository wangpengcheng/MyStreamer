...
Image::Image(uint8_t *data, int32_t width, int32_t height, int32_t stride, PixelFormat format, bool ownMemory) : mData(data), mWidth(width), mHeight(height), mStride(stride), mFormat(format), mOwnMemory(ownMemory)
{
    mSize = height * stride;
    mTimeStamp.tv_sec = 0;
